...
public:
    ...
    valueBucketMethod(int num, float U, vector<Element>& all_ele) : gen(rd()) {
        ...
        if (bucket_size > alias_threshold) {
            vector<double> tmp_bucket;
            for (int i = 0; i <= bucket_size; i++) {
                tmp_bucket.push_back(buckets[i].bucket_weight);
            }
            bucket_table = ConstantBucket(bucket_size, tmp_bucket);
        }
        //ele_size = num;
    }
    //randomly generate
